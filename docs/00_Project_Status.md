...
- Current task: Post-roadmap quality pass, P3 (query performance) — closed
- Status: Done
- Remaining error: None known
- Last commit: See `git log -1`
- Next task: Quality pass P4 (UX/UI polish) — design tokens and a type scale to replace the 59 lines of untouched Blazor-template CSS, the first `Components/Shared/` set (`StatusBadge`, `KpiCard`, `EmptyState`, `PageHeader`, and an alert that distinguishes success from failure), `table-responsive` wrappers, and the accessibility items. Watch the `InvariantCulture` rule: P4 touches many date cells and that is the easiest locked rule to break. Separately and unchanged: the user's own Day 14 portfolio work — Demo Video, updated CV, 30 job applications per §23.1 — is not a coding task; offer to help draft *content* if asked (a demo script already exists at `demo/talking-points.md`), but the applications/CV/video themselves are the user's. Everything the locked spec names through Day 14 is done: all 15 Required Tests, all 13 Release Verification items except IIS Local (environment finding, needs an elevated session on a machine with IIS), and the full locked canonical seed dataset (users/machines/materials/formulations/orders/plans/1 PR/1 Incoming PO).

...
| Bug audit round 7: CSV export **clean**; sign-in audit flattened every failure | Done | **CSV export — verified end to end, no defect.** Injection is genuinely reachable now that `UserAgent` is audited (this session added it) and that header is fully attacker-controlled. An anonymous caller sent `=cmd|'/c calc'!A1`, `@SUM(1+1)*cmd` and `+1,2"3` as User-Agent, each triggering a 401 audit row; the exported CSV renders all three apostrophe-prefixed and inert, and **parsing the whole file shows every row at exactly 9 columns** — the injected comma and quote did not shift a single column. UTF-8 BOM present. **Sign-in audit — real defect.** Every failed sign-in was recorded as `"Invalid credentials"`, so the audit could not distinguish a typo from a lockout, a deactivated account, or an unknown username. Reproduced live: six wrong passwords for `viewer.demo` locked the account (Identity's 5-attempt default, confirmed working — `LockoutEnd` set and **the correct password then refused**), and the audit showed seven identical rows. The seventh is the alarming one — somebody holding the right password, refused only because the account had just locked — and it read the same as the typos above it, on the module whose whole purpose is traceability. `DescribeOutcome` now records Locked out / Sign-in not allowed / Requires two-factor / Invalid credentials / Unknown username / Account is inactive. **Only the audit changed**: every case still returns the same redirect, so no username-enumeration signal is added, and the log is Admin/Manager-only. New `AuthenticationAuditTests` in its own class (locking an account costs 6 of the login limiter's 10 permits per minute); counterfactual fails it. `viewer.demo`'s lockout was cleared afterwards. 225/225 passed | **Considered and not changed:** the locked §13.1 formula-prefix set is `= + - @`; OWASP also lists leading tab and CR, but HTTP strips leading whitespace from header values and the other exported fields are code constants or Admin-controlled, so it is not reachable here. **Noted:** `LoginAsync` returns without hashing when the username is unknown, which is a timing side channel for username enumeration — standard mitigation is a dummy hash verify; low severity behind the 10/min per-IP login limiter | None |
| Bug audit round 6: shortage engine — **no defect found**, hardened instead | Done | Audited `MaterialRequirementQueryService` and the locked `MaterialAvailabilityRules` it delegates to. **No bug found**, which for the calculation the entire system is built around is the result worth recording. Confirmed by reading: `BuildTimeline` evaluates every distinct required date independently, so the locked correction "never collapse incoming supply evaluation to the minimum completion date across plans" holds; supply dated before today is excluded so late supply cannot mask a shortage; a negative on-hand position flows through un-floored as the rules require. **Output contract verified live on all 10 materials**: on-hand = stock − reserved, projected = on-hand + incoming, shortage = max(0, required − projected), a Material Required Date present exactly when short, shortage never negative — 0 violations. That live check only exercises the incoming path trivially (clock drift has made every PO late, so eligible incoming is 0 everywhere), so a **randomised property test** was added over 500 generated demand/supply sets with a fixed seed, covering the shapes nobody writes by hand: no demand, zero-quantity demand, past-dated demand and supply, negative on-hand, repeated dates. It asserts timeline dates strictly ascend, cumulative demand and cumulative incoming never fall, availability reconciles at every point, shortage equals the largest deficit, and the first shortfall date never follows the evaluation date. **Mutation-tested so it is not vacuous**: removing the `OrderBy` in `BuildTimeline` and swapping `FindIndex` for `FindLastIndex` are each caught with the exact diagnostic. 224/224 automated checks passed | **One design property checked and confirmed deliberate, not a defect:** `Evaluate` returns `MaterialRequiredDate` from the *first* shortfall date while every figure comes from the *max-deficit* date, so a list row can display a Required Date belonging to a different date than its quantities. `Shortage_grows_to_the_worst_date_not_the_first_one` already pins exactly this, the row still reconciles internally (required − projected = shortage), and the detail panel states the evaluation date explicitly | None |
| Bug audit round 5: master data store-exception gap + report view sweep | Done | **Master data — the duplicate-code fix never reached this service.** All three write paths (`CreateRawMaterialAsync`, `UpdateRawMaterialAsync`, `SaveFormulationAsync`) check for a duplicate `Code` with `AnyAsync` and then insert against a uniquely indexed column (`IX_RawMaterials_Code`, `IX_Formulations_Code`), so two callers submitting the same code concurrently both pass the pre-check and the index settles the race. `CustomerOrderService` identified exactly this, translates the store exception and documents why; `MasterDataService` translated only `DbUpdateConcurrencyException` on one of the three paths, so the loser of the race got a raw `DbUpdateException` — a 500 on the API and a dead circuit in the UI. Added a shared `SaveAsync` helper with the concurrency catch first (the ordering is load-bearing: `DbUpdateConcurrencyException` derives from `DbUpdateException`). Verified live afterwards: a duplicate still returns 400 from the pre-check, a **case-insensitive** duplicate (`rm-001` vs `RM-001`) is also caught because the database is `SQL_Latin1_General_CP1_CI_AS`, so pre-check and index agree on case; formulation round-trip still 200; 10 raw materials / 5 formulations intact. 223/223 automated checks passed | **Two hypotheses tested and rejected rather than reported:** (1) `SaveFormulationAsync` deletes and re-inserts `FormulationMaterials` rows carrying the same unique `(FormulationId, RawMaterialId)` key — EF orders the delete before the insert correctly, verified live with a round-trip PUT returning 200; (2) deactivating a raw material does **not** silently hide its demand, because the shortage engine does not filter on `IsActive`. **Report views — all four verified live:** each executes, and every property of its keyless entity is backed by a real view column (9/8/10/11 columns, no mismatches). **Recorded risk:** `vw_MaterialShortageReport` has **zero read sites** — `ExportMaterialShortageCsvAsync` deliberately uses `IMaterialRequirementQueryService` instead, for the documented reason that the view carries no multi-date math — so nothing in code or tests would notice if a migration broke that view, and EF does not track views (this already happened once, to `vw_AuditLogReport`). The InMemory test provider has no SQL engine, so views cannot be covered automatically; live verification is the only guard | None |
| Bug audit round 4: alert severity never updated + Program.cs sweep | Done | **Alert dedup — severity was frozen at whatever it was when the alert was raised.** `AlertEvaluationService` correctly updates an existing active alert in place instead of raising a second one, but the update branch assigned only `Message`, never `Severity`. A machine climbing from 86°C to 96°C keeps the same dedup key (`MachineTemperature`/`Machine`/id), so the row kept `Warning` while its own message changed to read "(Critical)". Two consequences: a single Dashboard row contradicting itself, and — worse — the alert list sorts Critical first, so a genuinely critical machine sorted *below* the real Critical alerts on the screen whose entire job is to surface the worst thing happening. The reverse direction was equally stuck, over-reporting a cleared condition. **Reproduced live on LocalDB** before the fix: `severity=Warning` against `Machine-02 is running at 96.0°C (Critical)`. New `AlertDeduplicationTests` (own fixture, so it cannot disturb siblings) drives Warning→Critical→Warning; counterfactual run fails with "Expected: Critical, Actual: Warning". Machine-02 restored to 95.0°C and the alert row now reads Critical; KPI unchanged at 10/8/1/1/2/2. 223/223 automated checks passed | **Program.cs reviewed and found clean:** `UseAuthorizationAudit` wraps `next` so it observes the final status despite being registered before `UseAuthorization`, and sits after `UseAuthentication` so it can name the user; both rate-limit policies partition on `RemoteIpAddress`, not on the user, so `UseRateLimiter` running before `UseAuthentication` is **not** the ordering bug it looks like; `IsMachineReadablePath` correctly covers `/hubs` as well as `/api` and `/health`; the CSP is written from `OnStarting` so it replaces rather than duplicates Blazor's own `frame-ancestors` header. The alert dedup filtered unique index is `(AlertType, EntityName, EntityId) WHERE IsActive = 1`, exactly matching the dedup key, so the `SingleOrDefault` lookup cannot throw. **POTENTIAL SECURITY ISSUE — measured, not changed:** `UseAuthorizationAudit` writes one row per 401/403 and only `/api/auth/login` is rate-limited, so an anonymous caller gets 1:1 write amplification on the fastest-growing table — **measured: 25 unauthenticated requests to `/api/customer-orders` produced exactly 25 audit rows**. The Copilot limiter already documents this exact concern ("a flood cannot turn itself into one audit write per request") but only on the authenticated path. Not fixed unilaterally because every remedy — suppressing duplicate (IP, path, status) within a window, or rate-limiting the anonymous API surface — changes what the locked Module 11 audit trail records or what the API returns | Decide on the audit write-amplification remedy |
| Bug audit round 3: Copilot output validation + SignalR broadcast | Done | **Copilot — missing output cap.** `CopilotResponseValidator` capped the summary (1000) and each recommendedActions item (300), and capped the *count* of affectedOrders at 20, but passed `int.MaxValue` as that array's per-item limit — so twenty unbounded strings passed validation and reached the page. The class doc already claimed "an item over its length/count cap — fails closed", and every sibling cap had both a constant and a unit test while this one had neither, which is what gave the omission away. `OllamaClient` reads the response body whole with no size limit, so this validator is the only bound the rendered answer has. Added `MaxAffectedOrderLength = 100` (an OrderNumber is capped at 30) and `MaxRawResponseLength = 16_000`, refusing an absurd payload before `JsonDocument.Parse` allocates for it. Three unit tests added; counterfactual run confirms both new guards fail without the fix. **SignalR — unguarded broadcast.** `SignalRMachineUpdateNotifier` wrapped its in-process subscriber loop in try/catch precisely so "one torn-down circuit must not fail the write that triggered the notification", but the `hub.Clients.All.SendAsync` line directly above it was unguarded. Every caller reaches it *after* its database write has committed, so a broadcast failure reported a machine update that had actually succeeded as a failure — and `OperationCanceledException` (a caller disconnecting cancels that token) is not a `UserFacingError`, so on the Blazor path it tore the circuit down. Now guarded and logged. Live push re-verified end to end after the change: an update pushed by curl from outside the browser moved an already-open Machine Monitoring page from 72.0°C to 75.0°C with no reload; machine restored and KPI still 10/8/1/1/2/2. 222/222 automated checks passed | Reviewed and found clean: the notifier's subscribe/unsubscribe lifecycle (`OnAfterRender(firstRender)` + flag-guarded `Dispose`, so prerender leaves nothing behind on the singleton); `CopilotRateLimiter` thread safety (`ConcurrentDictionary` + per-entry lock, correct zero-initialised window); deterministic role-filtered tool selection before the model is ever called; no raw-HTML rendering anywhere outside `NavIcon`. **Noted, not changed:** tool context is built from stored data, so a customer or material name is a stored-prompt-injection surface — bounded to advisory text by the structured-output validation and the read-only tool set, with no write path | None |
| Bug audit round 2: incoming-PO number race reported the wrong cause | Done | Continued the bug sweep beyond the audit module. Clean: no `DateTime.Now`/`UtcNow` anywhere outside `FixedTimeProvider`; no culture-sensitive number or date format missing `InvariantCulture` in either the `.ToString(...)` or interpolated form; `BatchSize` and `WeightPerBatch` both validated positive so `CalculateRequiredBatch`'s `ArgumentOutOfRangeException` guard is unreachable. **Found:** both generated business keys use `COUNT(*) + 1`, but only one is protected. `NextRequestNumberAsync` is called *inside* the locked Serializable transaction, whose range locks make it safe. `NextPurchaseOrderNumberAsync` is called *before* the transaction opens (`ProcurementService.cs:112` vs `:119`) and that transaction is ReadCommitted, so two concurrent creates for **different** purchase requests can both number themselves `PO-00000N`. `IX_IncomingPurchaseOrders_PurchaseOrderNumber` is unique so no duplicate persists — data integrity held — but the loser was told *"An incoming purchase order already exists for this purchase request"*, which is false and sends the operator to inspect the wrong record instead of retrying. The catch conflated two unique indexes; it now distinguishes them and reports the number collision honestly | **Established by reading the code, isolation levels and schema — not reproduced live, unlike the audit paging bug.** The race path has no automated coverage because EF InMemory does not enforce unique indexes, so the `DbUpdateException` never occurs there. **POTENTIAL BUSINESS LOGIC ISSUE, recorded not changed:** `COUNT(*) + 1` is fragile numbering regardless — raising this transaction to Serializable to match the PR path, or moving to a database sequence, would remove the collision rather than merely describe it. Both change isolation or schema, so neither was done in a bug-fix pass | None |
| Bug fix: audit log paging could repeat and drop rows | Done | **Real defect, proven on the canonical LocalDB before the fix.** `AuditLogService.ListAsync` sorted on `CreatedAt` alone and then applied `Skip`/`Take`. `AuditLogs.CreatedAt` is `datetime2(0)` (whole seconds), so rows written in the same second tie — 27 rows already contained 3 tie groups. With no unique final sort key the order among ties is the optimiser's, so running the *same* query under two valid plans (one per page, page size 10) returned **Id 17 on both pages and Id 18 on neither**. Worse under `FixedTimeProvider`, which returns a constant: in the `Demo` environment and every test the whole table is one tie group, and the log advertising newest-first actually returned **oldest-first**. Fixed by adding `.ThenByDescending(x => x.Id)` in `AuditLogService.ListAsync` and in `ReportExportService.ExportAuditLogCsvAsync` (where the 5,000-row cap otherwise took an undefined subset). Free: `Id` is the clustered key and therefore the locator in both audit indexes — `SHOWPLAN_TEXT` confirms **no Sort operator**, both shapes still `ORDERED BACKWARD` index access, so the P3 index work is not regressed. Re-ran the exact two-plan experiment after the fix: pages disjoint and complete. Live paging now returns 28-24 / 23-19 / 18-14 with the emitted SQL reading `ORDER BY [a].[CreatedAt] DESC, [a].[Id] DESC`. 218/218 automated checks passed | Also fixed while there: `AuditWriter` truncated `UserAgent` but wrote `IpAddress` straight through against an `nvarchar(45)` column, so the comment's own guarantee that an over-long value "must not be able to fail an audit write" held for only one of the two columns | None |
| Demo seed: full procurement status coverage | Done | The seed carried one Approved request and one Open order, leaving **5 of the 7 procurement statuses with no row behind them** — so screens built to show a lifecycle showed a single point on it, and the Executive Overview's "Awaiting Approval" figure read 0. `CanonicalProcurementSeeder` now seeds all four `PurchaseRequestStatus` values and all three `IncomingPurchaseOrderStatus` values (PR-BASE-002..006, PO-BASE-002/003), guarded per request number so an already-seeded database picks up the additions. **Every locked figure is unmoved, verified live: KPI 10/8/1/0, RM-001 required 5,000 / on-hand 3,750 / eligible incoming 0 / short 1,250.** That holds by construction — purchase requests are never read by the availability engine, only Open/Partial orders count as incoming supply so the one Partial row sits on RM-004 which has no shortage, and no added order is late at T. `ProcurementTests.Canonical_seed_covers_every_procurement_status_without_moving_the_locked_figures` asserts both halves. `deploy/seed-data.sql` regenerated and the SQL-only fallback path re-verified end to end on a throwaway database (schema + seed applied clean, same coverage, still 10 orders / 8 plans). 217/217 automated checks passed | **Three cases remain uncoverable without breaking a locked figure, recorded not fixed:** a late purchase order would move `LatePurchaseOrderCount` off 0; a Warning-risk order needs an 11th order, moving `CustomerOrderCount` off 10; and a non-zero Eligible Incoming is only observable on a material that is short, and the only short material is RM-001 whose 0 is the locked demo case | None |
...
| G24 | `/procurement` | **The receipt form could write one material's total against another.** `SelectOrder` seeded the cumulative quantity from `Items.First()` while the material select stayed on "Select material", so the number on screen belonged to a material the operator had not chosen — and stayed there when they chose a different one. The receipt contract is cumulative (locked), so submitting that pairing overwrites a running total rather than adding to it. Choosing a material now re-seeds the quantity from *that* material, with a hint spelling out that the box is a running total, not this delivery. | High | **Closed (C7)** |
| G25 | `/procurement` | Both tabs' datasets loaded on every page open though only one is rendered, each with a two-deep include chain — the same defect fixed as F4 on `/materials`. **Measured:** one `/procurement` load now issues queries against `PurchaseRequests` + `PurchaseRequestItems` only, and **zero** against `IncomingPurchaseOrders`/`IncomingPurchaseOrderItems`; requesting incoming orders directly still queries both, so the work moved rather than vanished. The Incoming tab deliberately loads both, because its create form picks from Approved requests. | Medium | **Closed (C7)** |
